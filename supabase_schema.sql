...
CREATE TABLE IF NOT EXISTS "public"."orders" (
  ...
  CONSTRAINT "orders_payment_status_check" CHECK (payment_status = ANY (ARRAY['pending'::text, 'pending_verification'::text, 'verified'::text, 'rejected'::text]))
);
...
ALTER TABLE ONLY "public"."orders" ADD CONSTRAINT "orders_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;
...
CREATE POLICY "Super Admins can update any profile" ON "public"."profiles" FOR UPDATE USING ((EXISTS (SELECT 1 FROM "public"."profiles" "profiles_1" WHERE (("profiles_1"."id" = "auth"."uid"()) AND ("profiles_1"."is_super_admin" = true)))));
