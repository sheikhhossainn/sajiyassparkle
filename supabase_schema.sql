...
CREATE
OR REPLACE FUNCTION "public"."add_admin"("target_email" "text") RETURNS json LANGUAGE "plpgsql" SECURITY DEFINER AS $ $ declare target_id uuid;

admin_count integer;

current_user_super boolean;

begin -- Check if caller is Super Admin
select
  is_super_admin into current_user_super
from
  profiles
where
  id = auth.uid();

if not current_user_super then return json_build_object(
  'success',
  false,
  'message',
...
if admin_count >= 2 then return json_build_object(
  'success',
  false,
  'message',
  'Maximum limit of 2 additional admins reached.'
);
...
if target_id is null then return json_build_object(
  'success',
  false,
  'message',
...
CREATE
OR REPLACE FUNCTION "public"."check_wishlist_limit"() RETURNS "trigger" LANGUAGE "plpgsql" AS $ $ DECLARE item_count integer;
...
IF item_count >= 5 THEN RAISE EXCEPTION 'Wishlist limit reached. You can only add up to 5 items.';
...
CREATE
OR REPLACE FUNCTION "public"."delete_user_account"() RETURNS "void" LANGUAGE "plpgsql" SECURITY DEFINER AS $ $ BEGIN -- Now a simple delete works because all dependencies cascade automatically
DELETE FROM
  auth.users
WHERE
  id = auth.uid();
...
CREATE
OR REPLACE FUNCTION "public"."handle_new_user"() RETURNS "trigger" LANGUAGE "plpgsql" SECURITY DEFINER AS $ $ DECLARE raw_full_name text;
...
IF raw_first_name IS NULL THEN IF raw_full_name IS NOT NULL THEN raw_first_name := split_part(raw_full_name, ' ', 1);

raw_last_name := split_part(raw_full_name, ' ', 2);

ELSE raw_first_name := split_part(new.email, '@', 1);
...
INSERT INTO
  public.profiles (id, email, username, is_admin, is_super_admin)
VALUES
  (
    new.id,
    new.email,
    COALESCE(
      raw_full_name,
      raw_first_name || ' ' || COALESCE(raw_last_name, ''),
...
CREATE
OR REPLACE FUNCTION "public"."remove_admin"("target_id" "uuid") RETURNS json LANGUAGE "plpgsql" SECURITY DEFINER AS $ $ declare current_user_super boolean;
...
select
  is_super_admin into target_super
from
  profiles
where
  id = target_id;

if target_super then return json_build_object(
  'success',
  false,
  'message',
  'Cannot remove Super Admin access.'
);
...
CREATE TABLE IF NOT EXISTS "public"."messages" (
  ...
  CONSTRAINT "messages_message_type_check" CHECK (
    (
      "message_type" = ANY (
        ARRAY ['General Inquiry'::"text", 'Product Question'::"text", 'Order Status'::"text", 'Custom Design'::"text", 'Feedback'::"text", 'Other'::"text"]
      )
    )
  ),
  CONSTRAINT "messages_status_check" CHECK (
    (
      "status" = ANY (
        ARRAY ['new'::"text", 'read'::"text", 'replied'::"text", 'archived'::"text"]
      )
    )
  )
);
...
ADD
  GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."order_items_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
  );
...
ALTER TABLE
  "public"."wishlist" OWNER TO "postgres";
...
ADD
  GENERATED BY DEFAULT AS IDENTITY (
    SEQUENCE NAME "public"."wishlist_id_seq" START WITH 1 INCREMENT BY 1 NO MINVALUE NO MAXVALUE CACHE 1
  );
...
CREATE
OR REPLACE TRIGGER "enforce_wishlist_limit" BEFORE
INSERT
  ON "public"."wishlist" FOR EACH ROW EXECUTE FUNCTION "public"."check_wishlist_limit"();
...
CREATE POLICY "Admins can update messages" ON "public"."messages" FOR
UPDATE
...
CREATE POLICY "Anyone can submit a message" ON "public"."messages" FOR
INSERT
  TO "authenticated",
...
CREATE POLICY "Super Admins can update any profile" ON "public"."profiles" FOR
UPDATE
  USING (
    (
      EXISTS (
...
CREATE POLICY "Users can insert into their own wishlist" ON "public"."wishlist" FOR
INSERT
  WITH CHECK (("auth"."uid"() = "user_id"));
...
GRANT ALL ON FUNCTION "public"."add_admin"("target_email" "text") TO "anon";

GRANT ALL ON FUNCTION "public"."add_admin"("target_email" "text") TO "authenticated";

GRANT ALL ON FUNCTION "public"."add_admin"("target_email" "text") TO "service_role";

GRANT ALL ON FUNCTION "public"."check_wishlist_limit"() TO "anon";

GRANT ALL ON FUNCTION "public"."check_wishlist_limit"() TO "authenticated";

GRANT ALL ON FUNCTION "public"."check_wishlist_limit"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."remove_admin"("target_id" "uuid") TO "anon";
